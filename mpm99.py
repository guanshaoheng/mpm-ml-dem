...
xn = 100
n_particles, n_grid = (xn+1)*(xn+1) * quality ** 2, 128 * quality
# n_particles, n_grid = 900, 50
...
def substep():
    ...
    for p in x:  # Particle state update and scatter to grid (P2G)
        ...
        if material[p] == 2:  # Reconstruct elastic deformation gradient after plasticity
            F[p] = U @ sig @ V.transpose()
        # The first Piola–Kirchhoff stress
        stress = 2 * mu * (F[p] - U @ V.transpose()) @ F[p].transpose() + \
                 ti.Matrix.identity(float, 2) * la * J*(J - 1)
        # Green-strain
        # stress = 2 * mu * 0.5 * (F[p] @ F[p].transpose() - ti.Matrix.identity(float, 2)) + \
        #          ti.Matrix.identity(float, 2) * la * J * (J - 1)
        # small strain
        # stress = 2 * mu * (0.5 * (F[p] + F[p].transpose()) - ti.Matrix.identity(float, 2)) + \
        #          ti.Matrix.identity(float, 2) * la * J * (J - 1)
    ...
    for i, j in grid_m:
        if grid_m[i, j] > 0:  # No need for epsilon here
            ...
            if i < 3 and grid_v[i, j][0] < 0:
                ...
                # grid_v[i, j][0] = -grid_v[i, j][0]  # Boundary conditions
            if i > n_grid - 3 and grid_v[i, j][0] > 0:
                grid_v[i, j][0] = 0
            if j < 3 and grid_v[i, j][1] < 0:
                grid_v[i, j][1] = 0
                # grid_v[i, j][1] = -grid_v[i, j][1]
...
print('-' * 100)
print('Number of particles: %d ' % n_particles)
print('Number of grids: %d ' % n_grid)
print('Groups size: %d ' % group_size)
...
def initialize():
    ...
    #         ti.random() * 0.2 + 0.1 + 0.21 * (i // group_size)
    #     ]
...
initialize()
gui = ti.GUI("Taichi MLS-MPM-9", res=512, background_color=0x112F41)
